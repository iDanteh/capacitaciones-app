...
export function LessonContentDrawer({
  ...
  // ── Shared header + tabs + body content ──────────────────────────────────

  const typeLabel =
    lesson.type === 'VIDEO' ? 'Lección de video' :
    lesson.type === 'TEXT'  ? 'Lección de texto' :
    'Archivo adjunto';

  const typeIcon =
    lesson.type === 'VIDEO' ? <Video    size={15} className="text-capta-soft" /> :
    lesson.type === 'TEXT'  ? <FileText size={15} className="text-emerald-600 dark:text-emerald-400" /> :
                              <File     size={15} className="text-amber-500" />;
    ...
    <div className="flex h-14 flex-shrink-0 items-center justify-between border-b border-border px-5">
      <div className="flex items-center gap-2.5 min-w-0">
        <div
          ...
          style={{
            background: lesson.type === 'VIDEO' ? '#8FC4E812' : lesson.type === 'TEXT' ? '#16a34a12' : '#f59e0b12',
          }}
        >
          {typeIcon}
        </div>
        <div className="min-w-0">
          <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground leading-tight">
            {typeLabel}
          </p>
          <p className="text-sm font-semibold text-foreground leading-tight truncate max-w-[240px]">
            {lesson.title}
          </p>
        </div>
      </div>
      <button
        onClick={onClose}
        className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
        title="Cerrar"
      >
        <X size={15} />
      </button>
  ...
  const tabBar = (
    <div className="flex border-b border-border px-5 gap-1 flex-shrink-0">
      {([
        ...
        >
          <Icon name={tab.icon as 'file' | 'shield'} size={12} />
          {tab.label}
        </button>
      ))}
    </div>
              ...
              <div className={`flex items-center gap-2.5 rounded-xl border px-4 py-3 ${
                ...
                <div>
                  <p className={`text-sm font-medium ${
                    lesson.muxStatus === 'ready'    ? 'text-emerald-600 dark:text-emerald-400' :
                    lesson.muxStatus === 'errored'  ? 'text-destructive' :
            ...
            <div>
              ...
              {uploading ? (
                <div className="rounded-2xl border border-capta-soft/30 bg-capta-soft/5 p-5 space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground">Subiendo a Mux… no cierres esta ventana.</p>
                </div>
              ) : (
                <label
                  onDragOver={e => e.preventDefault()}
                  onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) { setVideoFile(f); handleVideoUpload(f); } }}
                ...
                >
                  ...
                  <p className="text-sm font-medium text-foreground mb-1">Arrastra el video aquí, o haz clic para seleccionar</p>
                  <p className="text-xs text-muted-foreground">MP4 · MOV · MKV · WebM</p>
                  <input ref={videoInputRef} type="file" accept="video/*" className="hidden"
                    onChange={e => { const f = e.target.files?.[0]; if (f) { setVideoFile(f); handleVideoUpload(f); } }} />
                </label>
              )}
            </div>
          </div>
        ...
        {lesson.type === 'TEXT' && (
          loadingContent ? (
            ...
            <TextLessonSplitEditor
              ...
              onSave={newContent => { setContent(newContent); onUpdated({ content: newContent }); }}
              onClose={onClose}
            />
          )
        )}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {docUploading ? (
                ...
                <label
                  onDragOver={e => e.preventDefault()}
                  onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) { setDocFile(f); handleFileUpload(f); } }}
                ...
                >
                  ...
                  <input ref={docInputRef} type="file"
                    ...
                    className="hidden"
                    onChange={e => { const f = e.target.files?.[0]; if (f) { setDocFile(f); handleFileUpload(f); } }} />
                </label>
              )}
            </div>
          </div>
  ...
  // ── Panel variant (inline — desktop canvas) ───────────────────────────────

  if (variant === 'panel') {
    const showInlineEditor = lesson.type === 'TEXT' && activeTab === 'content';
    return (
  ...
  // ── Drawer variant (default — slide-over) ─────────────────────────────────
